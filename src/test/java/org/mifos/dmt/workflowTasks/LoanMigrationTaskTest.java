...
public class LoanMigrationTaskTest {
	
    LoanMigrationTask loanMigrationTask;
	...
	public void setUp() throws Exception
	{
		...
		loanMigrationTask=new LoanMigrationTask();
		  HashMap<String, String>stage1DbVal = DMTDatabaseConfigurations.STAGING_1;	  
			...
			String stage2jobFilePath= DMTConfig.DMT_CONFIG_DIR
			                         + "\\prepStaging.bat";
			String stage1Sql= "src/test/resources/org/mifos/dmt/workflowTasks/loan_schedule.sql";
    ...
    {    
		...
		boolean status=loanMigrationTask.execute();
    	IDatabaseConnection connection = databaseTester.getConnection();
        ITable actualTable = connection.createQueryTable("LOAN_SCHEDULE","select * from loan_schedule"); 
        ITable expectedTable =connection.createQueryTable("LOAN_SCHEDULE_TEST","select * from loan_schedule_test");
        ITable actualTableFee = connection.createQueryTable("LOAN_FEE_SCHEDULE","select * from loan_fee_schedule"); 
        ITable expectedTableFee =connection.createQueryTable("LOAN_FEE_SCHEDULE_TEST","select * from loan_fee_schedule_test");
