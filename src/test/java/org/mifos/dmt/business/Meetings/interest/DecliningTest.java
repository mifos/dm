...
public class DecliningTest {
	
	ArrayList<BigDecimal> principalPerInstallment;
	ArrayList<BigDecimal> interestPerInstallment;
  ...
  public void calculateLoanPayments() {
	  ...
	  while (itr.hasNext()) {
		...
		//calling of the Declining interest method
		...
		inputParams.setLoanDuration(loanDuration);
		inputParams.setLengthOfPeriod(lengthOfPeriod);
	    Declining declining = new Declining(inputParams);
	    declining.calculateLoanPayments();
	    principalPerInstallment=declining.getPrincipalPerInstallment();
	    interestPerInstallment=declining.getInterestPerInstallment();
	    ...
	    while (emiSet.hasNext()) {
			...
			Assert.assertTrue(principalPerInstallment.get(no).equals(principal.setScale(digitsAfterDecimal)));
			
			//calling the assert to check the expected and actual interest values of installment.
			Assert.assertTrue(interestPerInstallment.get(no).equals(Interest.setScale(digitsAfterDecimal)));
